...
handoffs:
  ...
  - label: Capture Implementation
    agent: evospec.capture
    prompt: Formalize what was built into an implementation spec
---
...
**This workflow is interactive and iterative.** The user may:
- Start with a vague idea and need help modeling entities and domains
- Want to explore design options before committing to an approach
- Iterate multiple times — refining the spec as they learn
- Build a prototype during discovery, then formalize later via `/evospec.capture`

**Do NOT force the user through a rigid pipeline.** Adapt to their confidence level:
- Low confidence → spend more time on empathy, ideation, entity modeling
- Medium confidence → focus on assumptions and experiment design
- High confidence → fast-track to tasks or suggest `/evospec.improve` instead
