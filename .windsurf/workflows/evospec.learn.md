...
5. **Decide next step (interactive)**:
...
- Weekly touchpoints with customers → cadence field in spec.yaml
- Small experiments, not big bets → one assumption per experiment
- Compare and contrast → multiple solution ideas in ideation
- Assumption mapping → structured assumptions with categories
- Opportunity solution tree → outcome → opportunity → solutions → assumptions
