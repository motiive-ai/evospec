---
description: Formalize what was built into an implementation spec + domain artifacts. Use after experimental work, prototyping, or any implementation you want to document retroactively.
handoffs:
  - label: Run Checks
    agent: evospec.check
  ...
  - label: Record Learnings
    agent: evospec.learn
    prompt: Record what was learned from this experiment
---
...
This workflow **formalizes work that already happened**. It's the complement to `/evospec.discover`:
- `/evospec.discover` helps you explore and start building
- `/evospec.capture` documents what you built so it can be reproduced, maintained, or handed off

**Two main use cases**:
1. **Post-experiment capture**: UX/product built a prototype during discovery. Now they want to
   document what exists — components, API integrations, state management, known gaps.
2. **Retroactive documentation**: Developer built something without formal specs. Now they want
   to create the implementation spec, entity definitions, and invariant mappings.

This workflow reads the actual codebase to generate documentation — it's not asking the user
to write docs manually. The AI scans the code and produces the spec.
...
- ALWAYS read the actual source code — do NOT ask the user to describe their implementation
- Generate ALL content — the user should not have to write docs manually
- Be honest about gaps — if invariants aren't enforced, say so clearly
