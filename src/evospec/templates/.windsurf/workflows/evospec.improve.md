...
1. **Find or create the spec directory**:
   ...
   - Parse the improvement description from `$ARGUMENTS`.
   - Generate a slug from the description.
   - Create directory: `specs/changes/YYYY-MM-DD-<slug>/`

2. **Classify the change** (same as discover, but `change_type: improvement`):
...
5. **Generate a brief scope document** (instead of discovery-spec.md):
...
6. **If zone is hybrid or core**, generate or update `domain-contract.md`:
...
7. **Auto-generate tasks** (improvements go straight to tasks):
   - Call `/evospec.tasks` or generate tasks.md directly
