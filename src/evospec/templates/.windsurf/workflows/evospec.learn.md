...
handoffs:
  - label: Discover (iterate)
    agent: evospec.discover
    prompt: Update the discovery spec based on new learnings
    send: true
  - label: Promote to Contract
    agent: evospec.contract
    prompt: Create a domain contract for validated assumptions
    send: true
---
...
3. Log the learning
4. Decide what's next: **continue**, **pivot**, **kill**, or **promote-to-core**

This is the most important workflow in the Discovery Layer. Without it, specs are static documents. With it, specs are living learning systems.
...
2. **Identify what was tested**:
   ...
   - If `$ARGUMENTS` contains experiment results, extract them.

3. **Record the experiment** (add to `discovery.experiments[]` in spec.yaml):
...
5. **Decide next step** (the critical decision):
   ...
   - Suggest running `/evospec.contract` to create a domain contract
   - Update feature knowledge_stage from `mystery` to `heuristic` or `algorithm`

6. **Log the learning** (add to `discovery.learnings[]` in spec.yaml):
   - `date`: today
...
7. **Update discovery status**:
   ...
   - If all high-risk assumptions validated → suggest moving to `/evospec.tasks`
...
8. **Report**:
   ...
   - If promote: "Run `/evospec.contract` to codify [assumption] as an invariant"

## Teresa Torres Continuous Discovery Habits

This workflow embodies:

1. **Weekly touchpoints with customers** → cadence field in spec.yaml
2. **Small experiments, not big bets** → one assumption per experiment
3. **Compare and contrast** → multiple solution ideas in ideation
4. **Assumption mapping** → structured assumptions with categories
5. **Opportunity solution tree** → outcome → opportunity → solutions → assumptions
