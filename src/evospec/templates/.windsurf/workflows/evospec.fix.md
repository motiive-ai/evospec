...
handoffs:
  - label: Generate Tasks
    agent: evospec.tasks
    prompt: Generate fix tasks from this bugfix spec
    send: true
  - label: Check Specs
    agent: evospec.check
...
2. **Root Cause Analysis** (before touching code):
...
- **Identify root cause before implementing** — never fix symptoms
...
- **If this reveals a spec gap**: add the missing invariant + fitness function
- **If an invariant exists but wasn't enforced**: fix the enforcement, not just the code
