...
2. **Parse the decision topic** from `$ARGUMENTS`:
   - Extract the core decision to be documented.
   - If no arguments provided, ask: "What decision do you want to record?"

3. **Determine next ADR number**:
   - Scan existing ADR files in the directory.
   - Find the highest number and increment by 1.

4. **Generate the ADR** using the template structure:
